...
def user_csv(apps, schema_edirot):
    ...
    UserInfo = apps.get_model('movies', 'UserInfo')
    for index, row in user_info.iterrows():
        ...
        UserInfo.objects.create(id=user_id, age=age, gender=gender, occupation=occupation, zip=zip_code)
...
def info_csv(apps, schema_editor):
    ...
    Movie = apps.get_model('movies', 'Movie')
    UserInfo = apps.get_model('movies', 'UserInfo')
    IDInfo = apps.get_model("movies", 'IdInfo')
    for index, row in id_info.iterrows():
        ...
        IDInfo.objects.create(user_id=user, movie_id=movie, rating=rating)
